"""Canonical quick-reference notebook generator CLI.

This is the primary entrypoint for generating notebooks under
`notebooks/quick_reference/`.
"""
...
def _build_notebook(
    ...
    nb.cells = [
        ...
        nbformat.v4.new_markdown_cell(
            f"## Learn More\nDetailed tutorial: `notebooks/tutorials/{tutorial_notebook}`"
        ),
...
source_df = pd.read_csv(ROOT / 'data' / 'ames_housing.csv').sample(n=80, random_state=42).reset_index(drop=True)
eval_df = source_df.copy()
booster = artifact._get_booster()
cat_cols = [str(col) for col in artifact.run_config.data.categorical if col in eval_df.columns]
for col, categories in zip(cat_cols, booster.pandas_categorical, strict=False):
    eval_df[col] = pd.Categorical(eval_df[col], categories=categories)
eval_result = evaluate(artifact, eval_df)

x_eval = eval_df.drop(columns=[target_col])
y_eval = eval_df[target_col].to_numpy(dtype=float)
...
artifact = Artifact.load(artifact_path)
target_col = str(artifact.run_config.data.target)
source_df = pd.read_csv(ROOT / 'data' / 'ames_housing.csv').sample(n=80, random_state=43).reset_index(drop=True)
reeval_df = source_df.copy()
booster = artifact._get_booster()
cat_cols = [str(col) for col in artifact.run_config.data.categorical if col in reeval_df.columns]
for col, categories in zip(cat_cols, booster.pandas_categorical, strict=False):
    reeval_df[col] = pd.Categorical(reeval_df[col], categories=categories)
feature_names = [str(c) for c in artifact.feature_schema.get('feature_names', [])]
missing_df = reeval_df.drop(columns=[feature_names[0]]) if feature_names else reeval_df.copy()
...
y_true = reeval_df[target_col].to_numpy(dtype=float)
y_pred = np.asarray(artifact.predict(reeval_df.drop(columns=[target_col])), dtype=float)
...
def _generate_legacy_notebooks(execute: bool = True) -> None:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="quick_reference",
        help=(
            "Generation target. "
            "'quick_reference' writes notebooks/quick_reference (canonical), "
            "'preview_alias' simulates the deprecated preview route, "
            "and 'all' is kept as a compatibility alias for quick_reference."
        ),
    )
    parser.add_argument(
        "--no-execute",
...
def main(argv: list[str] | None = None) -> None:
    ...
    if target in {"preview_alias", "quick_reference", "all"}:
        try:
            module = importlib.import_module("scripts.generate_quick_reference_notebook_specs")
        except ModuleNotFoundError:
            module = importlib.import_module("generate_quick_reference_notebook_specs")
        generate_quick_reference_notebooks = module.generate_quick_reference_notebooks
        if target == "preview_alias":
            print(
                "warning: --target preview_alias is deprecated; generating canonical "
                "notebooks/quick_reference instead.",
                file=sys.stderr,
            )
