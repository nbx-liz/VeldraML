"""Run end-to-end multiclass demo with fit/predict/evaluate."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Input CSV path prepared by prepare_demo_data_multiclass.py.",
    )
    parser.add_argument(
        "--out-dir",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not data_path.exists():
        raise SystemExit(
            ...
            "Hint: run `uv run python examples/prepare_demo_data_multiclass.py` first."
        )

    try:
        frame = load_tabular_data(str(data_path))
        if frame.empty:
            raise VeldraValidationError("Multiclass demo input data is empty.")
        if DEFAULT_TARGET not in frame.columns:
            raise VeldraValidationError(
                f"Multiclass demo input must include target column '{DEFAULT_TARGET}'."
            )
        ...
        train_path = run_dir / "train_data_multiclass.csv"
        ...
        raise SystemExit(
            format_error(
                exc,
                "Check data format, multiclass target values, and split settings before retry.",
            )
        ) from exc
    ...
    print(f"macro_f1: {eval_result.metrics['macro_f1']:.6f}")
