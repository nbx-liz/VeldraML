"""Run end-to-end frontier demo with fit/predict/evaluate."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Input CSV path prepared by prepare_demo_data_frontier.py.",
    )
    parser.add_argument(
        "--out-dir",
    ...
    parser.add_argument("--alpha", type=float, default=0.90, help="Quantile alpha for frontier.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not data_path.exists():
        raise SystemExit(
            ...
            "Hint: run `uv run python examples/prepare_demo_data_frontier.py` first."
        )

    try:
        frame = load_tabular_data(str(data_path))
        if frame.empty:
            raise VeldraValidationError("Frontier demo input data is empty.")
        if DEFAULT_TARGET not in frame.columns:
            raise VeldraValidationError(
                f"Frontier demo input must include target column '{DEFAULT_TARGET}'."
            )
        ...
        train_path = run_dir / "train_data_frontier.csv"
        ...
        pred = predict(artifact, test_df).data
        ...
        raise SystemExit(
            format_error(
                exc,
                "Check data format, alpha value, and split settings before retry.",
            )
        ) from exc
