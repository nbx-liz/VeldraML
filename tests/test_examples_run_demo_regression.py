...
from examples import run_demo_regression


def _demo_frame(rows: int = 120, seed: int = 7) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    x1 = rng.normal(size=rows)
    x2 = rng.normal(loc=2.0, scale=0.5, size=rows)
    y = 3.0 * x1 - 1.5 * x2 + rng.normal(scale=0.1, size=rows)
    return pd.DataFrame({"x1": x1, "x2": x2, "target": y})


def test_run_demo_regression_writes_expected_outputs(tmp_path) -> None:
    data_path = tmp_path / "demo.csv"
    out_dir = tmp_path / "out"
    _demo_frame().to_csv(data_path, index=False)

    exit_code = run_demo_regression.main(
        [
            "--data-path",
            str(data_path),
            "--out-dir",
...
def test_run_demo_regression_requires_prepared_csv(tmp_path) -> None:
    missing_path = tmp_path / "missing.csv"
    with pytest.raises(SystemExit) as exc_info:
        run_demo_regression.main(["--data-path", str(missing_path)])
