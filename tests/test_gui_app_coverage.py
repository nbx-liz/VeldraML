...
def test_app_coverage_edge_cases(monkeypatch):
    ...
    # Job with error
    job_err = GuiJobRecord(
        "j2", "failed", "fit", "now", "now", RunInvocation("fit"), None, error_message="Fatal Error"
    )
    ...
    job_payload = GuiJobRecord(
        "j3", "succeeded", "fit", "now", "now", RunInvocation("fit"), result=res_obj
    )
    ...
    # Exception
    monkeypatch.setattr(
        app_module, "cancel_run_job", lambda jid: (_ for _ in ()).throw(ValueError("CancelFail"))
    )
    ...
    monkeypatch.setattr(
        app_module, "list_artifacts", lambda p: (_ for _ in ()).throw(ValueError("ListFail"))
    )
    ...
    assert "Performance Metrics" in str(
        res_comp_err[1]
    )  # generic title, check repr of figure object or just str
    ...
    monkeypatch.setattr(
        app_module,
        "callback_context",
        types.SimpleNamespace(triggered=[{"prop_id": "config-validate-btn.n_clicks"}]),
    )
    ...
    monkeypatch.setattr(
        app_module, "validate_config", lambda y: (_ for _ in ()).throw(ValueError("ConfigFail"))
    )
    res_conf_ex = config_cb(1, 0, 0, 0, "yaml", "path", state)
    assert "ConfigFail" in res_conf_ex[1]

    # 11. _handle_migration_apply
    get_callback(app, "config-migrate-result.children")  # allow_duplicate=True
    ...
    monkeypatch.setattr(
        app_module, "migrate_config_file_via_gui", lambda p, target_version=1: "Migrated!"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda p, target_version=1: (_ for _ in ()).throw(ValueError("MigFail")),
    )
