...
def test_inspect_data_upload_csv():
    ...
    with patch("veldra.gui.app.inspect_data") as mock_inspect:
        ...
        mock_inspect.return_value = {
            "success": True, 
            "stats": {"n_rows": 2, "n_cols": 3, "columns": ["col1", "col2", "target"], "numeric_cols": [], "categorical_cols": [], "missing_count": 0},
            "preview": [{"col1": 1, "col2": 2, "target": 0}]
        }
        # Mock rendering functions inside data_page
        with patch("veldra.gui.pages.data_page.render_data_stats") as mock_stats:
            with patch("veldra.gui.pages.data_page.render_data_preview") as mock_prev:
                # Call callback
...
def test_build_config_yaml_causal_target_search_space():
    ...
    yaml_str = _cb_build_config_yaml(
        "regression", "data/path.csv", "target_col", [], [], [], # task, data...
        "kfold", 5, 42, None, None, "expanding", None, 0, 0, # split...
        0.1, 31, 100, -1, 20, 100, 1.0, 1.0, 0, 0, # train...
        True, "standard", 30, "rmse", # tune
        "artifacts", # export
        True, "dr", # causal
        0.01, 0.2, 16, 64, None, None, None, None # search space
    )
...
def test_populate_builder_options():
    ...
    with patch("veldra.gui.app.inspect_data") as mock_inspect:
        ...
        path, target, opts_id, opts_cat, opts_drop, opts_grp, opts_time = _cb_populate_builder_options("/", state)
