...
def test_inspect_data_upload_csv(mock_makedirs, mock_open, mock_inspect):
    ...
    mock_inspect.return_value = {
        "success": True, 
        "stats": {"columns": ["A"], "n_rows": 10, "n_cols": 1, "numeric_cols": [], "categorical_cols": [], "missing_count": 0}, 
        "preview": [{"A": 1}]
    }
...
def test_build_config_yaml_full():
    ...
    yaml_str = _cb_build_config_yaml(
        "regression", "data.csv", "target", ["id"], ["cat"], ["drop"],
        "timeseries", 5, 42, "group_col", "time_col", "rolling", 0.2, 1, 1, # split params
        0.1, 31, 100, -1, 20, 10, 1.0, 1.0, 0, 0, # train params
        True, "custom", 20, "rmse", "artifacts", # tuning
        True, "dr", # causal
        "0.01", "0.1", "10", "50", "3", "10", "0.5", "1.0" # search space
    )
