...
def test_config_actions(monkeypatch):
    ...
    # Error in validation
    monkeypatch.setattr(
        app_module, "validate_config", lambda y: (_ for _ in ()).throw(ValueError("Invalid"))
    )
...
def test_migration_apply(monkeypatch):
    ...
    monkeypatch.setattr(
        app_module, "migrate_config_file_via_gui", lambda p, target_version=1: "Success"
    )

    res = apply_cb(1, "path", 1)
    assert "Success" in str(
        res
    )  # html.Div(html.Pre("Success")) -> str usually shows structure or we can inspect children

    # Error
    monkeypatch.setattr(
        app_module,
        "migrate_config_file_via_gui",
        lambda p, target_version=1: (_ for _ in ()).throw(ValueError("Fail")),
    )
