...
from examples import evaluate_demo_artifact
from veldra.api import fit


def _frame(rows: int = 80, seed: int = 21) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    x1 = rng.normal(size=rows)
    x2 = rng.normal(size=rows)
    y = 1.2 * x1 + 0.8 * x2 + rng.normal(scale=0.05, size=rows)
    return pd.DataFrame({"x1": x1, "x2": x2, "target": y})


def _train_artifact(tmp_path) -> str:
    train_path = tmp_path / "train.csv"
    _frame().to_csv(train_path, index=False)
    run = fit(
        {
            "config_version": 1,
...
def test_evaluate_demo_artifact_writes_metrics(tmp_path) -> None:
    artifact_path = _train_artifact(tmp_path)
    eval_data_path = tmp_path / "eval.csv"
    _frame(rows=40, seed=22).to_csv(eval_data_path, index=False)
    out_dir = tmp_path / "eval_out"

    exit_code = evaluate_demo_artifact.main(
        [
            "--artifact-path",
            artifact_path,
            "--data-path",
...
def test_evaluate_demo_artifact_requires_target_column(tmp_path) -> None:
    artifact_path = _train_artifact(tmp_path)
    bad_data_path = tmp_path / "bad_eval.csv"
    _frame(rows=20, seed=23).drop(columns=["target"]).to_csv(bad_data_path, index=False)

    with pytest.raises(SystemExit) as exc_info:
        evaluate_demo_artifact.main(
            [
                "--artifact-path",
                artifact_path,
                "--data-path",
